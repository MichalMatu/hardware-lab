...
static uint8_t s_usb_netif_driver_handle;

static void usb_net_set_link(bool up) {
    tud_network_link_state(TINYUSB_USBDEV_0, up);
}
...
static void usb_net_init_callback(void *ctx) {
    (void)ctx;
    usb_net_set_link(true);
}
 ...
 *
 *  From the PC's NIC perspective the L2 forwarding should be transparent and resemble this
 * configuration:
 *
 *   (ISP) router                           PC
 *      [ AP ]       <---------->       [ virtual wifi-NIC ]
 ...
 *  No need to modify the ethernet frames here, as we can set the station's MAC to the USB NCM
 * device.
 */
...
esp_err_t wired_bridge_init(wired_rx_cb_t rx_cb, wired_free_cb_t free_cb) {
    ...
    tinyusb_net_config_t net_config = {
        ...
        .on_init_callback = usb_net_init_callback,
    };
...
/**
 *  In this scenario of configuring WiFi, we setup USB-Ethernet to create a virtual network and run
 * DHCP server, so it could assign an IP address to the PC
 *
 *           ESP32               PC
 ...
 *      | (wifi-provisioning) |
 *
 *  From the PC's NIC perspective the board acts as a separate network with it's own IP and MAC
 * address, but the virtual ethernet NIC has also it's own IP and MAC address (configured via
 * tinyusb_net_init()). That's why we need to create the virtual network with *different* MAC
 * address. Here, we use two different OUI range MAC addresses.
 */
esp_err_t wired_netif_init(config_access_mode_t access_mode, esp_netif_t **out_netif) {
    ...
    esp_netif_driver_ifconfig_t driver_cfg = {
        .handle = &s_usb_netif_driver_handle,
        .transmit = netif_transmit,      // point to static Tx function
        .driver_free_rx_buffer = l2_free // point to Free Rx buffer function
    };

    // 3) USB-NCM is an Ethernet netif from lwip perspective, we already have IO definitions for
    // that:
    struct esp_netif_netstack_config lwip_netif_config = {
        .lwip = {.init_fn = ethernetif_init, .input_fn = ethernetif_input}};
    ...
    esp_netif_config_t cfg = {
        .base = &base_cfg, .driver = &driver_cfg, .stack = &lwip_netif_config};
    ...
    uint8_t router_offer = config_access_mode_uses_gateway(access_mode) ? 1 : 0;
    ret = esp_netif_dhcps_option(s_netif, ESP_NETIF_OP_SET, ESP_NETIF_ROUTER_SOLICITATION_ADDRESS,
                                 &router_offer, sizeof(router_offer));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Cannot set USB config default route option");
        return ret;
    }

    uint8_t dns_offer = config_access_mode_uses_dns(access_mode) ? 1 : 0;
    ret = esp_netif_dhcps_option(s_netif, ESP_NETIF_OP_SET, ESP_NETIF_DOMAIN_NAME_SERVER,
                                 &dns_offer, sizeof(dns_offer));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Cannot set USB config DNS option");
        return ret;
    }

    uint32_t lease_opt = 1;
    ret = esp_netif_dhcps_option(s_netif, ESP_NETIF_OP_SET, ESP_NETIF_IP_ADDRESS_LEASE_TIME,
                                 &lease_opt, sizeof(lease_opt));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Cannot set USB config DHCP lease time");
        return ret;
    }
