...
def should_regenerate_output_file():
    # Always rebuild when EMBED_WWW is enabled to ensure fresh frontend
    if not flag_exists("EMBED_WWW"):
        return False
    if not exists(output_file):
        return True
    
    # Force rebuild for upload tasks to ensure latest frontend is embedded
    import sys
    if 'upload' in sys.argv:
        print("Upload detected - forcing frontend rebuild")
        return True
...
def build_progmem():
    mimetypes.init()
    # Force recompilation by touching ESP32SvelteKit.cpp which includes WWWData.h
    esp32sveltekit_cpp = project_dir + "/lib/framework/core/ESP32SvelteKit.cpp"
    if exists(esp32sveltekit_cpp):
        os.utime(esp32sveltekit_cpp, None)
