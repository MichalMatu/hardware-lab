...
constexpr uint8_t BINARY_RECORD_SIZE = 16;
...
// Single log record (16 bytes total - compiler optimized)
// All numeric fields are in little-endian (ESP32 native)
// Note: Even with __attribute__((packed)), compiler pads to 16 bytes for alignment
struct __attribute__((packed)) BinaryLogRecord {
    ...
    uint16_t lux;          // Light intensity (0 to 65535 lux) - 2 bytes
    uint16_t salt;         // Soil conductivity (0 to 2000 µS/cm) - 2 bytes
    uint16_t batVolt_mv;   // Battery voltage in millivolts (0 to 5000 mV) - 2 bytes
    uint8_t soil;          // Soil moisture (0 to 100%) - 1 byte
    uint8_t batPerc;       // Battery percentage (0 to 100%) - 1 byte
    // Total: 16 bytes (compiler aligns to 16-byte boundary)
};

// Compile-time size validation
static_assert(sizeof(BinaryFileHeader) == 8, "BinaryFileHeader must be 8 bytes");
static_assert(sizeof(BinaryLogRecord) == 16, "BinaryLogRecord must be 16 bytes");
