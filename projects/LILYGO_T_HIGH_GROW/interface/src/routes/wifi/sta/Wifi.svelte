<script lang="ts">
	...
	const toolbarIconClass = 'h-6 w-6 text-base-content';
	...
	async function postWiFiSettings() {
		try {
			console.log('[WiFi] Preparing to post settings', wifiSettings);

			const sanitizedNetworks = wifiSettings.wifi_networks.map((network) => {
			...
			const payload = {
				...wifiSettings,
				hostname: wifiSettings.hostname || 'esp32',
				wifi_networks: sanitizedNetworks
			};

			console.log('[WiFi] POST /rest/wifiSettings payload', payload);
			...
			console.log('[WiFi] Response status', response.status);
			if (response.status == 200) {
				...
				console.error('[WiFi] Failed response body', body);
				notifications.error(`Failed to update Wi-Fi settings. (${response.status})`, 5000);
			}
		} catch (error) {
			console.error('[WiFi] POST error', error);
	...
	function checkHostname() {
		if (!wifiSettings.hostname) {
			wifiSettings.hostname = 'esp32';
		}
	...
	function applyWifiSettings() {
		if (checkHostname()) {
			if (wifiSettings.wifi_networks.length === 0) {
				// No networks configured - will go directly to AP mode
				modals.open(toModalComponent(ConfirmDialog), {
					title: 'No Networks Configured',
					message:
						'ESP32 will restart and enter AP mode immediately.\n\n' +
						'• No WiFi networks configured\n' +
						'• Device will start in AP mode\n' +
						'• Connect to AP SSID to reconfigure',
					labels: {
						cancel: { label: 'Cancel', icon: Cancel },
						confirm: { label: 'Apply & Restart to AP', icon: Check }
					},
					onConfirm: () => {
						postWiFiSettings();
						modals.close();
				...
				// Networks configured - will try to connect
				modals.open(toModalComponent(ConfirmDialog), {
					title: 'Apply WiFi Settings?',
					message:
						'ESP32 will restart and attempt to connect to configured networks.\n\n' +
						'• 3 connection attempts per network\n' +
						'• If all networks fail → AP mode activated\n' +
						'• AP SSID: Check your AP settings',
					labels: {
						cancel: { label: 'Cancel', icon: Cancel },
						confirm: { label: 'Apply & Restart', icon: Check }
					},
					onConfirm: () => {
						postWiFiSettings();
						modals.close();
	...
	function scanForNetworks() {
		modals.open(toModalComponent(ScanNetworks), {
	...
	function handleNewNetwork(ssid?: string) {
		modals.open(toModalComponent(EditNetwork), {
			title: 'Add network',
			networkEditable: {
				ssid: ssid || '',
				password: '',
	...
	function isNetworkListTooLong() {
		if (wifiSettings.wifi_networks.length >= 5) {
			modals.open(toModalComponent(InfoDialog), {
...
{:then _result}
	<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
				...
				<div class="card-body p-4 space-y-3">
					<div class="flex flex-col gap-2">
						<div class="flex items-center justify-between gap-3">
							...
							<div
								class="badge badge-lg {wifiStatus.status === 3 ? 'badge-success' : 'badge-outline'}"
							>
						...
						>
							<div class="rounded-box bg-base-100 flex items-center space-x-3 px-4 py-2">
								<div class="flex h-10 w-10 items-center justify-center">
									<AP class={statusIconClass(wifiStatus.status === 3)} />
								</div>
								<div>
				...
				<div class="card bg-base-200 shadow-primary/50 shadow-lg">
					<div class="card-body p-4">
						<h2 class="card-title text-lg mb-2">
							<Settings class="w-6 h-6" /> Connection Settings
						</h2>
						<div class="space-y-3">
							<div>
							...
							<div>
								<label class="label" for="apmode">Station (STA) Mode</label>
		...
		<!-- Right Column: Configured Networks -->
		{#if !page.data.features.security || $user.admin}
			<div class="card bg-base-200 shadow-primary/50 shadow-lg">
				<div class="card-body p-4">
					<h2 class="card-title text-lg mb-2">
						<Router class="w-6 h-6" /> Configured Networks
					</h2>
					...
					<div transition:slide|local={{ duration: 300, easing: cubicOut }}>
						...
						{:else}
							<DraggableList
								...
								class="space-y-2"
							>
								{#snippet children({ item: network, index: _index }: { item: any; index: number })}
