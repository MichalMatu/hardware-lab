<script lang="ts">
	...
	function validateNetworkSettings() {
		...
		// Validate Password (empty = open network, otherwise WPA2-PSK 8-63 chars)
		if (
			networkEditable.password.length > 0 &&
			(networkEditable.password.length < 8 || networkEditable.password.length > 63)
		) {
			valid = false;
						...
						{#if formErrors.password}
							<div transition:slide|local={{ duration: 300, easing: cubicOut }}>
								<label for="pwd" class="label">
									<span class="text-error"> Password must be empty or 8-63 characters long. </span>
								</label>
							</div>
						{/if}
					</div>
