...
/**
 * Format a date/time string for short display (navbar) without timezone conversion.
 */
export function formatShortDateTime(dateString: string): string {
	try {
		const parsed = parseIsoWithOffset(dateString);
		if (!parsed) throw new Error('invalid ISO');
		return `${pad2(parsed.day)}.${pad2(parsed.month)} ${pad2(parsed.hour)}:${pad2(parsed.minute)}`;
	} catch (error) {
		console.error('Error formatting date:', dateString, error);
		return '';
	}
...
/**
 * Format a date/time string into short parts (date + time) without timezone conversion.
 * Useful for responsive UIs where date and time are shown/hidden independently.
 */
export function formatShortDateTimeParts(dateString: string): { date: string; time: string } {
	try {
		const parsed = parseIsoWithOffset(dateString);
		if (!parsed) throw new Error('invalid ISO');
		return {
			date: `${pad2(parsed.day)}.${pad2(parsed.month)}`,
...
export function formatUTCDateTime(dateString: string): string {
	try {
		...
		console.error('Error formatting UTC date:', dateString, error);
		return 'Invalid date';
	}
}
